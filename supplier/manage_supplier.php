...
$filter_id = isset($_GET['supplierId']) ? trim($_GET['supplierId']) : '';
...
$where = "WHERE 1=1";
if ($f_id !== '')
  $where .= " AND s.id LIKE '%$f_id%'";
if ($f_name !== '')
  $where .= " AND s.name LIKE '%$f_name%'";
if ($f_phone !== '')
  $where .= " AND s.phoneNo1 LIKE '%$f_phone%'";
if ($f_email !== '')
  $where .= " AND s.emailId LIKE '%$f_email%'";
if ($filter_active !== '') {
  $active_val = ($filter_active === 'Yes') ? 1 : 0;
  $where .= " AND s.active = $active_val";
}
if ($f_city !== '')
  $where .= " AND a.city LIKE '%$f_city%'";
if ($f_zip !== '')
  $where .= " AND a.zipCode LIKE '%$f_zip%'";

$count_result = $conn->query("SELECT COUNT(*) as total FROM supplier s LEFT JOIN address a ON s.address = a.id $where");
$total_records = (int) $count_result->fetch_assoc()['total'];
$total_pages = $total_records > 0 ? ceil($total_records / $limit) : 1;

$sql = "SELECT s.id, s.name, s.phoneNo1, s.phoneNo2, s.whatsAppNo, s.emailId,
               s.active, a.line1, a.line2, a.city, a.zipCode, a.id as address_id
        FROM supplier s
        LEFT JOIN address a ON s.address = a.id
        $where ORDER BY s.id ASC LIMIT $limit OFFSET $offset";
...
<head>
  ...
  <title>Manage Suppliers – Sunder Machines World</title>
  <link
...
<body>
  ...
  <?php if (isset($_GET['saved'])): ?>
    <div class="toast show" id="savedToast"><i class="fa fa-check-circle"></i>&nbsp; Supplier saved successfully!</div>
    <script>setTimeout(function () { document.getElementById('savedToast').classList.remove('show'); }, 3000);</script>
  <?php endif; ?>

  <?php if (isset($_GET['deleted'])): ?>
    <div class="toast show" id="deletedToast" style="background:#e67700"><i class="fa fa-trash"></i>&nbsp; Supplier
      deleted.</div>
  ...
  <div class="erp-container">
    <div class="erp-header-bar">
      <div class="erp-header-title"><i class="fa fa-truck" style="margin-right:8px"></i>Manage Suppliers</div>
      <div class="erp-header-actions">
        <button type="button" class="btn-erp btn-erp-new" onclick="openNewSupplier()">
    ...
    <form method="GET" action="" id="filterForm">
      ...
      <div class="filter-group">
        ...
        <input type="text" name="supplierId" id="f_supplierId" value="<?= htmlspecialchars($filter_id) ?>"
          placeholder="Search ID…" maxlength="20">
      </div>
      <div class="filter-group">
        <label>Name</label>
        <input type="text" name="name" id="f_name" value="<?= htmlspecialchars($filter_name) ?>"
          placeholder="Search name…" maxlength="100">
      </div>
      <div class="filter-group">
        <label>Phone #</label>
        <input type="text" name="phoneNo1" id="f_phoneNo1" value="<?= htmlspecialchars($filter_phone) ?>"
          placeholder="10-digit number" maxlength="10">
      </div>
      <div class="filter-group">
        <label>Email ID</label>
        <input type="text" name="emailId" id="f_emailId" value="<?= htmlspecialchars($filter_email) ?>"
          placeholder="Search email…" maxlength="150">
      </div>
      <div class="filter-group">
        <label>Active</label>
        <select name="active">
      ...
      <div class="filter-group">
        <label>City</label>
        <input type="text" name="city" id="f_city" value="<?= htmlspecialchars($filter_city) ?>" placeholder="City…"
          maxlength="100">
      </div>
      <div class="filter-group">
        <label>Zip Code</label>
        <input type="text" name="zipCode" id="f_zipCode" value="<?= htmlspecialchars($filter_zip) ?>"
      ...
      <div class="filter-actions">
        ...
        <a href="manage_suppliers.php" class="btn btn-grey"><i class="fa fa-xmark"></i> Clear</a>
      </div>
    </div>
  </form>
        ...
        <?php if (empty($suppliers)): ?>
          <tr>
            <td colspan="9" style="text-align:center;padding:30px;color:var(--muted)">
              <i class="fa fa-inbox" style="font-size:1.5rem;margin-bottom:6px;display:block"></i>No suppliers found.
            </td>
          </tr>
        <?php else: ?>
          <?php $i = $offset + 1;
          foreach ($suppliers as $s): ?>
            <tr>
              ...
              <td><?php $ap = array_filter([$s['line1'] ?? '', $s['city'] ?? '']);
              echo htmlspecialchars(implode(', ', $ap)); ?>
              </td>
              <td><?= htmlspecialchars($s['phoneNo1'] ?? '') ?></td>
              <td><?= htmlspecialchars($s['whatsAppNo'] ?? '') ?></td>
              <td><?= htmlspecialchars($s['emailId'] ?? '') ?></td>
              <td><span class="<?= $s['active'] ? 'badge-active' : 'badge-inactive' ?>"><?= $s['active'] ? 'Yes' : 'No' ?></span>
              </td>
              <td>
                <div class="action-cell">
                  <button class="icon-btn address" title="Address"
                    onclick='showAddress(<?= htmlspecialchars(json_encode($s), ENT_QUOTES) ?>)'><i
                      class="fa fa-location-dot"></i></button>
                  <button class="icon-btn edit" title="Edit"
                    onclick='openEdit(<?= htmlspecialchars(json_encode($s), ENT_QUOTES) ?>)'><i
                      class="fa fa-pen"></i></button>
                  <button class="icon-btn delete" title="Delete" onclick='confirmDelete(<?= $s['id'] ?>)'><i
  ...
  <div class="pagination-bar">
    ...
    <div class="pag-btns">
      ...
      <button class="pag-btn" <?= $page <= 1 ? 'disabled' : '' ?> onclick="location.href='<?= $qsp ?>page=<?= $page - 1 ?>'">Previous</button>
      <span class="pag-btn active" style="background:var(--green); color:#fff; font-weight:bold;"><?= $page ?></span>
  ...
  <div class="modal-overlay" id="addressModal">
    <div class="modal" style="max-width:420px">
      <div class="modal-header">
        <h2><i class="fa fa-location-dot" style="margin-right:8px"></i>Supplier Address</h2>
  ...
  <div class="modal-overlay" id="deleteModal">
    <div class="modal" style="max-width:380px">
      <div class="modal-header">
        <h2 style="color:var(--red)"><i class="fa fa-triangle-exclamation" style="margin-right:8px"></i>Confirm Delete
        </h2>
        <button class="close-btn" onclick="closeModal('deleteModal')">✕</button>
      </div>
      <div class="modal-body">
        <p style="color:var(--text);font-size:.95rem;line-height:1.6">Are you sure you want to delete this supplier?
          This action <strong>cannot be undone</strong>.</p>
      </div>
      <div class="modal-footer">
        <button class="btn btn-grey" onclick="closeModal('deleteModal')">Cancel</button>
        <button class="btn" style="background:var(--red);color:#fff" id="confirmDeleteBtn"><i class="fa fa-trash"></i>
          Delete</button>
      </div>
    </div>
  </div>
  ...
  <div class="modal-overlay" id="editModal">
    <div class="modal" style="max-width:580px">
      <div class="modal-header">
        <h2 id="editModalTitle"><i class="fa fa-pen" style="margin-right:8px"></i>Edit Supplier</h2>
          ...
          <div class="form-grid">
            ...
            <div class="form-group full">
              ...
              <div class="toggle-wrap">
                <label class="toggle"><input type="checkbox" name="active" id="edit_active" value="1"><span
  ...
  <script>
    ...
    function submitFilter() {
      ...
      if (phone !== '' && !/^\d{10}$/.test(phone)) {
        showFilterError('f_phoneNo1', 'Enter a valid 10-digit phone number'); ok = false;
      } else { clearFilterError('f_phoneNo1'); }

      var zip = document.getElementById('f_zipCode').value.trim();
      if (zip !== '' && !/^\d{6}$/.test(zip)) {
        showFilterError('f_zipCode', 'Enter a valid 6-digit ZIP/PIN'); ok = false;
      } else { clearFilterError('f_zipCode'); }

      var email = document.getElementById('f_emailId').value.trim();
      if (email !== '' && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
        showFilterError('f_emailId', 'Enter a valid email address'); ok = false;
    ...
    function showFilterError(fieldId, msg) {
      ...
      el.classList.add('is-invalid'); el.title = msg; el.setAttribute('placeholder', msg);
    }
    ...
    var RULES = {
      edit_name: {
        ...
        label: 'Name',
        patternMsg: 'Only letters, numbers and common punctuation (2–100 chars)'
      },
      edit_phoneNo1: {
        required: true,
        pattern: /^\d{10}$/,
      ...
      edit_line1: {
        ...
        patternMsg: 'Invalid characters or too short'
      },
      edit_line2: {
        required: false,
        pattern: /^[^<>{}|\\^`]{0,200}$/,
      ...
      edit_city: {
        ...
        label: 'City',
        patternMsg: 'Only letters and spaces allowed (min 2 chars)'
      },
      edit_zipCode: {
        required: true,
        pattern: /^\d{6}$/,
    ...
    function validateField(id) {
      var rule = RULES[id]; if (!rule) return true;
      var el = document.getElementById(id);
      var err = document.getElementById('err_' + id.replace('edit_', ''));
      var val = el.value.trim();
      if (rule.required && val === '') { setError(el, err, rule.label + ' is required'); return false; }
      if (!rule.required && val === '') { clearError(el, err); return true; }
      if (!rule.pattern.test(val)) { setError(el, err, rule.patternMsg); return false; }
      clearError(el, err); return true;
    }
    function setError(el, errEl, msg) { el.classList.add('is-invalid'); el.classList.remove('is-valid'); if (errEl) errEl.textContent = msg; }
    function clearError(el, errEl) { el.classList.remove('is-invalid'); el.classList.add('is-valid'); if (errEl) errEl.textContent = ''; }
    function clearFormErrors() {
      Object.keys(RULES).forEach(function (id) {
        var el = document.getElementById(id), key = id.replace('edit_', ''), err = document.getElementById('err_' + key);
    ...
    function submitEditForm(e) {
      e.preventDefault();
      var valid = true;
      Object.keys(RULES).forEach(function (id) { if (!validateField(id)) valid = false; });
      if (!valid) {
      ...
      fetch('supplier_save.php', { method: 'POST', body: fd })
        .then(function (r) { return r.text(); })
        .then(function () {
          closeModal('editModal');
          location.href = 'manage_supplier.php?saved=1';
        })
    ...
    function showAddress(s) {
      ...
      document.getElementById('ad_line2').textContent = s.line2 || '—';
      document.getElementById('ad_city').textContent = s.city || '—';
      document.getElementById('ad_pin').textContent = s.zipCode || '—';
      document.getElementById('ad_phone').textContent = s.phoneNo1 || '—';
    ...
    function openEdit(s) {
      ...
      document.getElementById('edit_address_id').value = s.address_id || '';
      document.getElementById('edit_name').value = s.name || '';
      document.getElementById('edit_phoneNo1').value = s.phoneNo1 || '';
      document.getElementById('edit_phoneNo2').value = s.phoneNo2 || '';
      document.getElementById('edit_whatsAppNo').value = s.whatsAppNo || '';
      document.getElementById('edit_emailId').value = s.emailId || '';
      document.getElementById('edit_active').checked = (s.active == 1);
      document.getElementById('activeLabel').textContent = (s.active == 1) ? 'Active' : 'Inactive';
      document.getElementById('edit_line1').value = s.line1 || '';
      document.getElementById('edit_line2').value = s.line2 || '';
      document.getElementById('edit_city').value = s.city || '';
      document.getElementById('edit_zipCode').value = s.zipCode || '';
      openModal('editModal');
    }
    ...
    document.getElementById('confirmDeleteBtn').addEventListener('click', function () {
      ...
      fetch('supplier_delete.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: 'id=' + encodeURIComponent(deleteTargetId)
      })
        .then(function () { location.href = 'manage_supplier.php?deleted=1'; })
